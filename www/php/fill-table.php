...
if (isset($_GET['code']))
{
  ...
  if($code=='2'){
    $code='SELECT GROUP_ID, GROUP_TITLE as "Наименование", GROUP_PREFIX as "Префикс" FROM TAST_GROUP e where GROUP_CAPTION = (SELECT MAX(CAPTION_ID) FROM TAST_CAPTION)';
  }                 
  if($code=='3'){
  ...
  if($code=='4'){
    ...
    MAN_CAPTION = (SELECT MAX(CAPTION_ID) FROM TAST_CAPTION)';
  }  
  if($code=='5'){
